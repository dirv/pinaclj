...
(defn- get-build-page [page-path]
  (let [fs-root (test-fs/create-file-system)]
    (let [page (build-page (nio/child-path fs-root page-path) fs-root)]
      page)))
...
(describe "build-page"
  (it "creates a string containing the content"
    (should-contain "yellow cup green cup" (get-build-page "coffee ritual"))))
